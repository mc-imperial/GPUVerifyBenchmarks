//pass
//--local_size=[64,64] --num_groups=[8,8]
...
{
	__requires(width == 64*8 /*get_global_size(0)*/);
	__requires(mulFactor == 1024);
    ...
    for(i = 0;
            __invariant(__write_implies(gaussianRand, __write_offset_bytes(gaussianRand)/sizeof(float4) >= actualPos)),
            __invariant(__write_implies(gaussianRand, __write_offset_bytes(gaussianRand)/sizeof(float4) < actualPos + mulFactor)),
        i < mulFactor / 2; ++i)
    {
