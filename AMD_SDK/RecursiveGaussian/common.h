				       ...
				       const float coefp, const float coefn)
{
    ...
    for (int y = height - 1;
         __global_invariant(__implies(__write(output), ((__write_offset_bytes(output)/sizeof(uchar4))%width)==x)),
         __global_invariant(__implies(__read(output), ((__read_offset_bytes(output)/sizeof(uchar4))%width)==x)),
         y > -1; y--) 
    {
        int pos = x + y * width;
        float4 xc =  (float4)(input[pos].x, input[pos].y, input[pos].z, input[pos].w);
