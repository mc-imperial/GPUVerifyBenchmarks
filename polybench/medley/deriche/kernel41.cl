...
__kernel void kernel41(__global float *c2, __global float *imgOut, __global float *y1, __global float *y2, int h, int w)
{
  __requires(h == 256);
  __requires(w == 1024);
    long b0 = get_group_id(0), b1_0 = get_group_id(1);
    long t0 = get_local_id(0), t1 = get_local_id(1);
    __local float shared_c2;
        ...
        if (w >= t0 + c0 + 1)
          for (long c1_0 = 32 * b1_0; c1_0 < h; c1_0 += 8192) {
            ...
            for (long c3 = t1; c3 <= min(31, h - c1_0 - 1); c3 += 16)
              imgOut[(t0 + c0) * h + (c1_0 + c3)] = (shared_c2 * (y1[(t0 + c0) * h + (c1_0 + c3)] + y2[(t0 + c0) * h + (c1_0 + c3)]));
          }
