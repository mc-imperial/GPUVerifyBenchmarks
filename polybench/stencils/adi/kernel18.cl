...
__kernel void kernel18(__global double *q, __global double *v, int n, int tsteps, long c0)
{
  __requires(n == 1024);
  __requires(tsteps == 512);
    ...
    #define floord(n,d) (((n)<0) ? -((-(n)+(d)-1)/(d)) : (n)/(d))
    {
      ...
      __function_wide_invariant(__read_implies(v, ((((((((32) * (b0)) + (t0)) >= (0)) & ((((32) * (b0)) + (t0)) <= (1048575))) & (((((__read_offset_bytes(v)) / (sizeof(double))) / (n)) % (n)) == (0))) & ((n) >= ((((__read_offset_bytes(v)) / (sizeof(double))) % (n)) + (2)))) & ((((__read_offset_bytes(v)) / (sizeof(double))) % (n)) >= (1))) & ((((((32) * (b0)) + (t0)) - (((__read_offset_bytes(v)) / (sizeof(double))) % (n))) % (1048576)) == (0))));
    }
}
